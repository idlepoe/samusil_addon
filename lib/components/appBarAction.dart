...
List<Widget> AppBarAction(BuildContext context, Profile profile) {
  ...
  return [
    Column(
      ...
      children: [
        badges.Badge(
          badgeContent: Text(noReadLength.toString(),
    ...
    PopupMenuButton<PopupItem>(
      icon: CircleAvatar(
          backgroundImage: Utils.isValidNilEmptyStr(profile.profile_image_url)
              ? NetworkImage(profile.profile_image_url)
              : null,
          child: Utils.isValidNilEmptyStr(profile.profile_image_url)
              ? null
              : const CircleAvatar(
      ...
      itemBuilder: (BuildContext context) => <PopupMenuEntry<PopupItem>>[
        ...
        PopupMenuItem<PopupItem>(
          ...
          child: Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              Arrays.getBoardInfo(Define.INDEX_PROFILE_PAGE).icon,
              const SizedBox(width: 10),
              Text(Arrays.getBoardInfo(Define.INDEX_PROFILE_PAGE).title.tr),
            ],
          ),
        ),
        // PopupMenuItem<PopupItem>(
